...
test('W58: a CRITICAL bow shot beside the owner\'s own pull, and his numbers are untouched', { skip: SKIP58 }, () => {
  ...
  // 313 when this window was cut, and short by the second-person DoT the fork's parser fix
  // recovered: `You have taken N damage from <Spell> by <caster>.` used to fail the `has taken`
  // gate outright (DOT_RE, log/parseCombat.ts; tests/combatIncomingDot.test.mts). Two such lines
  // here, 58 points, and 313 + 58 = 371. The bow lane's own numbers above are unmoved, which is
  // this test's actual subject.
  assert.equal(inTotal, 371)
})
